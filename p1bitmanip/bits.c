...
int byteSwap(int x, int n, int m) {
    int n_eight = n<<3;
    int m_eight = m<<3;
    int hole = (~(255<<m_eight)&(x&(~(255<<n_eight))));
    int n_byte = ((x>>n_eight) & 255)<<m_eight;
    int m_byte = ((x>>m_eight) & 255)<<n_eight;
    return hole|n_byte|m_byte;
}
...
int isAsciiDigit(int x) {
    int is_less = !((x+(~0x30+1))>>31);
    int is_greater = !((0x39+(~x+1))>>31);
 ...
 */
int isPower2(int x) {
    int neg_one = 0xFF<<24 | 0xFF<<16 | 0xFF<<8 | 0xFF;
    int pow_check = (x + neg_one)&x;
    int zero = !!x^(x>>31);
    return (!pow_check) & zero;
}
...
int rotateLeft(int x, int n) {
    int last_n_bits = (x>>(32+(~n+1))&((1<<n) + ~0));
    int push_up = x<<n;
    return last_n_bits | push_up;
}
